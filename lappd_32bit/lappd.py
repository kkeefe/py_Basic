...
# lappd.py
#
# Interface for the register controller of the lappd
#
...
def loadHeaders():
    ...
    # Search an environment variable for LAPPD_SRC_PATH
    mungypath = os.environ.get('LAPPD_SRC_PATH')
    if mungypath is None:
        print("Error: LAPPD_SRC_PATH is not defined.  Please export a path to lappd_regmap.h and lappd_control.h")
        exit(1)
    ...
    try:
        registers = open(path + "/lappd_regmap.h")
    except Exception as e:
        print("Could not open lappd_regmap.h", e, file=sys.stderr)
        raise
    ...
    for n,line in enumerate(registers):
        ...
        try:
            ...
            print("lappd_regmap.h:%d does not appear to be a simple assignment.  Skipping.\n \t%s" % (n, line), e, file=sys.stderr)
            
    registers.close()

    # Load the control map and protocol definition
    try:
        commands = open(path + "/lappd_control.h")
    except Exception as e:
        print("Could not open lappd_control.h", e, file=sys.stderr)
        raise
    ...
    for n,line in enumerate(commands):
        ...
        try:
            ...
            print("lappd_control.h:%d does not appear to be a simple assignment.  Skipping. \n \t%s" % (n, line), e, file=sys.stderr)
...
def discover(broadcast, version="cafe0002", timeout=0.1):
    ...
    s.sendto((LAPPD_MAGIC_COOKIE).to_bytes(4, 'big') + bytes.fromhex("00000000 ffffffff"), (broadcast, int(LAPPD_SERVER_PORT)))
...
class payload:
    ...
    def makeRegisterWrite(regDict=None, silent=False):
        tmp = payload(LAPPD_OP_MASK_REG & (LAPPD_WRITE | (LAPPD_OP_MASK_SILENT if silent else 0x0)))
    ...
    def makeRegisterRead(regDict=None, silent=False):
        tmp = payload(LAPPD_OP_MASK_REG & (LAPPD_READ | (LAPPD_OP_MASK_SILENT if silent else 0x0)))
        if not regDict is None:
    ...
    def registerPopulate(load, regdict):
        ...
        for key,value in regdict.items():
            registerbytes.extend(key.to_bytes(LAPPD_WIDTH_REGISTER, 'big'))
            registerbytes.extend(value.to_bytes(LAPPD_WIDTH_REGISTER, 'big'))
    ...
    def bigBytesToReglist(pairsinbytes):
        ...
        for k in range(0, int(len(pairsinbytes) / (2*LAPPD_WIDTH_REGISTER))):
            offset = k*2*LAPPD_WIDTH_REGISTER
    ...
    def __init__(self, op, payload=None):
        ...
        if not isinstance(op, int):
            raise TypeError("Operation must be a %d-byte wide integer" % LAPPD_WIDTH_OP)
...
class board:
    ...
    def __init__(self, machine):
        ...
        try:
            ...
            self.s.connect((self.dest, LAPPD_SERVER_PORT))
        except Exception as e:
    ...
    def peek(self, arg1):
        ...
        if isinstance(arg1, dict):
            ...
            raise LAPPDException("Could not interpret %s in any sensible way" % (arg1))
    ...
    #
    def poke(self, arg1, arg2=None, silent=False):
        ...
        if isinstance(arg1, dict) and arg2 is None:
            ...
            raise LAPPDException("Could not interpret %s and %s in any sensible way" % (arg1, arg2))
    ...
    # 
    def pokenow(self, addr, word, silent=False):
    ...
    def transact(self):
        ...
        if self.dest is None:
            error = LAPPDException("Not yet pointed to a target board")
            error.board = self
            raise error

        # Are there any pending transactions?
        if len(self.transactions) == 0:
            raise LAPPDException("There are no pending transactions.")
        ...
        # Cookie
        frame.extend(LAPPD_MAGIC_COOKIE.to_bytes(4, 'big'))

        # Version
        # (note that the first bitshift makes a 4-byte wide dig)
        frame.extend( ((LAPPD_VERSION_HARD << 16) | LAPPD_VERSION_SOFT).to_bytes(4, 'big'))

        # Message ID
        # We'll check this later when we get a response
        random.seed()
        msgid = random.randrange(0, 1 << 8*LAPPD_WIDTH_SEQNUM)
        frame.extend( msgid.to_bytes(LAPPD_WIDTH_SEQNUM, 'big'))
        ...
        for action in self.transactions:
            ...
            if (action.op & LAPPD_OP_MASK_SILENT) == 0:
                response += 1
            
        # If we exceed maximum length, clear the request and except
        if len(frame) > LAPPD_MAX_PAYLOAD:
            error = LAPPDException("Length (%d) of assembled outgoing LAPPD frame exceeds header-defined maximum (%d).  Discarded." % (len(frame), LAPPD_MAX_PAYLOAD))
            error.board = self
            raise error
        ...
        while attempts > 0:
            ...
            if ready[0]:
                ...
                if not addr[0] == self.dest:
                    raise LAPPDException("Expected response from %s, but received response from %s" % (self.dest, addr))
                
                # Verify that the message has the correct magic
                # (note that we undo network byte ordering by just reading it backwards)
                if not int.from_bytes(data[0:4], byteorder='big') == LAPPD_MAGIC_COOKIE:
                    raise LAPPDException("Received magic %s that was not in lappd protocol" % data[0:4].hex())

                # Verify that the message has the correct version
                if not int.from_bytes(data[4:8], byteorder='big') == ((LAPPD_VERSION_HARD << 16) | LAPPD_VERSION_SOFT):
                    raise LAPPDException("Received version %s, expecting %s" % (data[4:8].hex(), msgid+1))

                    
                # Verify that the message has the correct message id
                if not int.from_bytes(data[8:12], byteorder='big') == msgid+1:
                    raise LAPPDException("Received msgid %d, expecting msgid %d" % (int.from_bytes(data[8:12], byteorder='big'), msgid+1))
        ...
        if attempts == 0:
            raise LAPPDException("Responsed timed out repeatedly, giving up")
        ...
        for n in range(0, len(self.transactions)):
            ...
            if self.transactions[n].op & LAPPD_OP_MASK_SILENT > 0:
                continue
            ...
            if not self.transactions[n].op == op:
                raise LAPPDException("Received unexpected operation %s instead of %s" % (op.hex(), transaction[n].op.hex()))

            # Verify message width integrtiy
            if not len(self.transactions[n].payload) == width:
                raise LAPPDException("Received a payload width %d not in agreement with expectation %d" % (width, len(transaction[n].payload)))

            # Did this operation fail?  If so, append its index to the failures list
            if op & LAPPD_OP_MASK_FAILURE > 0:
            ...
            if op & LAPPD_OP_MASK_REG > 0:

                # Make sure the width makes sense
                if width % (LAPPD_WIDTH_REGISTER*2) > 0:
                    raise LAPPDException("Width of register operation list %d did not divide correctly" % width)

                # Get the number of registers here
                nreg = int(width / (LAPPD_WIDTH_REGISTER*2))
