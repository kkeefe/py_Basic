...
// This file is never committed to the repo.  Create it locally.
//
// The following platforms are supported: SPARTAN6, SERIES7
// The following platforms may become supported: ZYNQ
//
// I've not been able to divine how SDK communicates to the build what version of Xilinx standalone is
// is being offered, not what our HW platform is... so we're going to do it like this.
//
// Create this file, and #define a single one of the above symbols
...
//
struct lappd_register {

  u32 addr;
  u32 word;
};
  
struct lappd_payload {

  //
  // So that many registers can be set at once without wasting an additional u32
  // each time.
  //
  // If op & LAPPD_OP_MASK_REG, then the following interpretation:
  //   width = N*sizeof(struct lappd_register)
  //   payload contains N lappd_register structures
  //   op & LAPPD_OP_MASK_OTHER = 0x0
  // If any of the checks fail, the operation is silently ignored.
  //
  // If op & LAPPD_OP_MASK_SILENT, then this operation will
  // not be echoed in the transaction response
  //
  // If op & LAPPD_OP_MASK_OTHER, then
  //    board specific controls can be implemented.
  //    op & LAPPD_OP_MASK_REG = 0x0
...
struct lappd {
  ...
  struct lappd_payload transaction[0];
  
  //   -- DATABLOCK --
};

// #define's for the lappd access have been pushed to lappd_control.h
