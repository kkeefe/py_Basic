...
void readFsl(void) {
  ...
  if (!tempInvalid) {
    ...
    // OOO: This is only well behaved if MAX_PACKET_SIZE % 4 == 0
  ...
  if (tempLast) {
    ...
    // Did we overflow?
    // (it will never exceed it, because we stop incrementing)
    if (gInboundWordPos == MAX_PACKET_SIZE / 4) {
...
static void udp_input(struct NIFT_source *source, struct udphdr * udp_packet) {
  ...
  if (Xil_Ntohs(udp_packet->uh_dport) == LAPPD_SERVER_PORT) {
    ...
    if(udp_packet->len < sizeof(struct lappd))
      return;

    // Okay.  Attempt to parse.
    source->sport = Xil_Ntohs(udp_packet->uh_sport);
    lappd_input(source, (struct lappd *)((char *)udp_packet + sizeof(struct udphdr)), udp_packet->len);
    return;
  }
...
//
static void* safe_memcpy(void *dest, void *src, int N, void *boundary) {
...
//
static int os_handler(u16 reg, u32 tmp) {
...
static void lappd_input(struct NIFT_source *source, struct lappd *lappdhdr_in, int transaction_len) {
  ...
  struct lappd *lappdhdr_out;
  struct lappd_payload *payloadhdr_in, *payloadhdr_out;
  struct lappd_register *regptr;
  ...
  lappdhdr_out = (struct lappd *)(((char *)udp_header) + sizeof(struct udphdr));

  // This will get incremented as we go along, but set it to the first one
  payloadhdr_out = (struct lappd_payload *)(((char *)lappdhdr_out) + sizeof(struct lappd));
  ...
  lappdhdr_out->seqnum = Xil_Ntohl(lappdhdr_in->seqnum) + 1;
  lappdhdr_out->seqnum = Xil_Htonl(lappdhdr_out->seqnum);
  ...
  read_boundary = (u8 *)lappdhdr_in->transaction + transaction_len - sizeof(struct lappd);
  ...
  payloadhdr_in = lappdhdr_in->transaction;
  ...
  if(Xil_Ntohl(lappdhdr_in->version) != lappd.version)
    ...
    while((u8 *)payloadhdr_in < read_boundary) {
      ...
      if( (op & LAPPD_OP_MASK_REG) && !(op & LAPPD_OP_MASK_OTHER)) {
	...
	if(width % sizeof(struct lappd_register)) {
	  free(packet - HAX);
	  return;
	}

	// Integer division to get the number of register operations
	N = width / sizeof(struct lappd_register);
	
	// Perform writes?
	if( (op & LAPPD_OP_MASK_REG) == LAPPD_WRITE) {
      
	  // Iterate through the register operations
	  for(regptr = (struct lappd_register *) (payloadhdr_in->payload); N > 0; --N, ++regptr) {
	...
	if(!(op & LAPPD_OP_MASK_SILENT) || ( (op & LAPPD_OP_MASK_REG) == LAPPD_READ) ) {
	  ...
	  if(!safe_memcpy(payloadhdr_out, payloadhdr_in, sizeof(struct lappd_payload) + width, boundary)) {
	  ...
	  len += sizeof(struct lappd_payload) + width;
	  
	  // Iterate through again and read out the registers
	  // and "erase and fill in the blanks"
	  for(regptr = (struct lappd_register *) (payloadhdr_out->payload); N > 0; --N, ++regptr) {
	  ...
	  payloadhdr_out = (struct lappd_payload *) ( (u8 *)payloadhdr_out + sizeof(struct lappd_payload) + width);
	}
      }
      else if( (op & LAPPD_OP_MASK_OTHER) && !(op & LAPPD_OP_MASK_REG) ) {

	// Not yet implemented.
	// Could be things like release a dhcp lease, or release a seized IP address
	...
	return;
      ...
      payloadhdr_in = (struct lappd_payload *) ( (u8 *)payloadhdr_in + sizeof(struct lappd_payload) + width );
...
static void icmp_input(struct NIFT_source *source, struct icmphdr *icmp_packet, struct ip *ip_packet) {
  ...
  if(icmp_packet->type == ICMP_ECHO) {
    ...
    packet = (u8 *)memalign(4, sizeof(struct ether_header) + ip_packet->ip_len + HAX);
    if(!packet)
      return;
    ...
    // Clear it
    memset(packet, 0, boundary - packet);

    // Index into the packet
    ip_header = (struct ip *)(packet + sizeof(struct ether_header));
    icmp_header = ( struct icmphdr *)((char *)ip_header + sizeof(struct ip));
 ...
 static void arp_cache_push(u32 ip, u8 *mac) {
 ...
 static int arp_cache_resolve(u32 ip, u8 *mac) {
   ...
   packet += HAX;
   memset(packet, 0, sizeof(struct ether_header) + sizeof(struct arphdr));
   arp_packet = (struct arphdr *)(packet + sizeof(struct ether_header));
 ...
 static void arp_input(struct arphdr *arp_packet) {
  ...
  /* // Are we configured yet?  If not ... */
  /* if(NIFT_ipsystem.configured < 2) { */

  /*   // If we're not in the middle of configuring... */
  /*   if(!NIFT_ipsystem.configured) { */

  /*     // Then we can try to steal */
  /*   } */
  /*     return; */
  /* } */
...
static void ip_output(struct ip *ip_header, u32 dst, int proto, int *len) {
...
int main(void) {
  ...
  // of the version
  lappd.version = result << 16;

  // And then OR in the the software version as the low 4 bytes 
  lappd.version |= (LAPPD_VERSION_MASK_SOFT & LAPPD_VERSION_SOFT);
